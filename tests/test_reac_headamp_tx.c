...
int main(void)
{
	...
	CHK(reac_headamp_tx_set(&t, 0, REAC_HEADAMP_SWEEP_RECORDS_PER_CH, 0) == -1);                 /* bad param */
	CHK(reac_headamp_tx_set(&t, 0, REAC_HEADAMP_PHANTOM, 2) == -1); /* bad bool  */
	CHK(reac_headamp_tx_set(&t, 0, REAC_HEADAMP_SENS, REAC_HEADAMP_SENS_STEPS) == -1); /* bad sens  */
	...
	CHK(reac_headamp_tx_set(&hi, REACPW_HEADAMP_TOP_CH, REAC_HEADAMP_PHANTOM, 1) == 0);  /* S-1608 in 16 */
	...
	CHK(reac_headamp_tx_next(&hi, &ch, &p, &v) == 1);
	CHK(ch == REACPW_HEADAMP_TOP_CH && p == REAC_HEADAMP_PHANTOM && v == 1);
	...
	while (records < REAC_BOX_S0808_IN * REAC_HEADAMP_NPARAMS && calls < REAC_BOX_S0808_IN * REAC_HEADAMP_SWEEP_RECORDS_PER_CH * REAC_HEADAMP_SWEEP_STRIDE + 8) {
		calls++;
	...
	records = 0;
	for (int i = 0; i < REAC_BOX_S0808_IN * REAC_HEADAMP_SWEEP_RECORDS_PER_CH * REAC_HEADAMP_SWEEP_STRIDE + 8 && records < REAC_BOX_S0808_IN * REAC_HEADAMP_SWEEP_RECORDS_PER_CH; i++) {
		if (!reac_headamp_tx_next(&rp, &ch, &p, &v))
			continue;
		records++;
	...
	CHK(records == REAC_BOX_S0808_IN * REAC_HEADAMP_SWEEP_RECORDS_PER_CH);
	...
	for (int i = 0; i < REAC_BOX_S0808_IN * REAC_HEADAMP_SWEEP_RECORDS_PER_CH * REAC_HEADAMP_SWEEP_STRIDE + 8 && records < REAC_BOX_S0808_IN * REAC_HEADAMP_SWEEP_RECORDS_PER_CH; i++)
		if (reac_headamp_tx_next(&rp, &ch, &p, &v))
			records++;
	CHK(records == REAC_BOX_S0808_IN * REAC_HEADAMP_SWEEP_RECORDS_PER_CH);   /* the full scene still went out */
	...
	records = 0;
	for (int i = 0; i < REAC_BOX_S1608_IN * REAC_HEADAMP_SWEEP_RECORDS_PER_CH * REAC_HEADAMP_SWEEP_STRIDE + REAC_BOX_S1608_IN; i++)
	...
	CHK(REAC_HEADAMP_RESWEEP_SECONDS == 0);
	{
		...
		int shipped = 0;
		for (int i = 0; i < REAC_BOX_S0808_IN * REAC_HEADAMP_SWEEP_RECORDS_PER_CH * REAC_HEADAMP_SWEEP_STRIDE + 32; i++)
			if (reac_headamp_tx_next(&ship, &ch, &p, &v))
				shipped++;
		CHK(shipped == 1 + REAC_BOX_S0808_IN * REAC_HEADAMP_SWEEP_RECORDS_PER_CH);          /* the edge, then the complete scene */
	...
	int scene = 0;
	for (int i = 0; i < REAC_BOX_S0808_IN * REAC_HEADAMP_SWEEP_RECORDS_PER_CH * REAC_HEADAMP_SWEEP_STRIDE + 32 && scene < REAC_BOX_S0808_IN * REAC_HEADAMP_SWEEP_RECORDS_PER_CH; i++)
		if (reac_headamp_tx_next(&rs, &ch, &p, &v))
			scene++;
	CHK(scene == REAC_BOX_S0808_IN * REAC_HEADAMP_SWEEP_RECORDS_PER_CH);
