...
**Status: implemented** (`reac_headamp_tx`, the ESTABLISHED FILLER overlay in
`reac_pacer`). **Rig-unproven** — the gate is at the end of this file.
...
`REAC_HEADAMP_RESWEEP_SECONDS` is **2 s**. It is a POLICY, stated as a named
constant with that word in its comment: an operator who wants a slower refresh on
a congested segment, or a faster one for a rig that is losing records, changes
this and nothing else.

The cost is small enough that the number is not delicate. Records ride
`REAC_HEADAMP_SWEEP_STRIDE` (12) frames apart, on FILLER slots whose control
block is otherwise idle — no audio moves, and no probe/grant/chanmap/cfea frame
is ever displaced (the pacer's overlay guard is unchanged). A 16-input box with
every cell set is 48 records: ~576 frames = **72 ms** of every 2 s at 8000 fps,
about 0.3 % of slots carrying a control block they would otherwise waste. A desk
with nothing set emits nothing at all.
...
one. If a box turns out to glitch its preamp on a rewrite, the mitigation is the
constant above, not a redesign: raise it, or set it to 0 and the behaviour is
exactly the old one.
...
Until step 1 has been run against a real box, this file's Status line says
rig-unproven and stays that way.
