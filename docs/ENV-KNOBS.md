...
| `REACPW_LINK_MBIT=<n>` / `REACPW_LINK_MBIT_<port>=<n>` | What the PHYSICAL port carries, in Mbit/s, for the link-budget admission that refuses a second REAC master on one wire. OVERRIDES `/sys/class/net/<port>/speed`, which reports a veth as 10 Gbit, does not exist in a network namespace, and on a real NIC reports the NEGOTIATION and not a slower uplink behind it. `<port>` is the physical port's name, never a VLAN's — a VLAN's budget is its parent's. `0` or unparseable means unknown, and unknown never refuses. | the port's own `speed`, or unknown |
