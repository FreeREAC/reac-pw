...
| `REAC_ROLE` / `REAC_ROLE_<segment>` | Which end of the desk<->stagebox pairing we present as on a segment: `master`, `slave` or `auto`. **`auto` is the default and the product path**: the daemon listens first and resolves per segment — a desk mastering the wire is joined as a slave, a wire with a box on it and no master is taken as master after a 3 s hunt window and granted, a stagebox strapped to master is refused and logged, never fought. The PER-SEGMENT key (`REAC_ROLE_enp131s0=master`) overrides the wire; the bare key is only a FLOOR for a segment nobody has heard yet and the hunt supersedes it, naming the layer it came from. `--role` on a `--live` invocation is unchanged. | `auto` (the wire decides) |
