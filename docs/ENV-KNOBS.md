...
| `REACPW_PACER=thread\|etf` | Master role: which backend owns the instant a frame leaves. `thread` is `clock_nanosleep` + `sendto`, the egress instant being this thread's wake. `etf` puts a `SCM_TXTIME` launch time on every frame off an exact `CLOCK_TAI` grid and lets the kernel's ETF qdisc release it, so the thread only has to be EARLY rather than punctual. `etf` REFUSES TO OPEN and names the code when its preconditions are missing (an ETF qdisc on the device, `SO_TXTIME` — which needs `CAP_NET_ADMIN` — and a non-zero kernel TAI offset); it never falls back quietly, because a run that believes it is measuring launch-time pacing while the thread paces is worse than no run. Per-segment (`REACPW_PACER_<iface>`), so one segment can run each arm for a comparison. See libreac's `docs/ETF-PACING.md` for the `tc` commands. | `thread` |
| `REACPW_PACER_LEAD_US=<n>` | Master role, `etf` only: how far ahead of a frame's launch time the thread hands it to the kernel, in microseconds, 50..50000. The default is derived — 2000 µs of measured worst wake tail (this pacer's 30-minute soak) plus a 300 µs qdisc `delta` — not chosen. A lead is buffered audio, so longer is only more latency; shorter than the worst tail is a lead the thread will miss. | 2500 |
